...
export const createArticle = (req, res) => {
    ...
    const { title, description, price, type, category,userId, imgUrl } = req.body;
    if (!title || !description || !price|| !type  || !category || !userId || !imgUrl) {
        return req
            .status(400)
            .json({ message: 'Missing required parameter: category' }, err);
    }
    // Construct the SQL query for inserting data
    const query = 'INSERT INTO articles (userId , description, price, title, type,  imgUrl, category) VALUES (?, ?, ?, ?, ?, ?, ?);';

    // Execute the query with the provided article details
    db.query(query, [userId , description, price, title, type,  imgUrl, category], (err, data) => {
        if(err) {
        ...
        res.status(201).json({
         ...
         userIdId:// Assuming data has insertId which is typical for SQL insert operations
            title,
            description,
            price,
            type,
